...
static IGNORABLE_FILES: &[&str] = &[
    "cue", "gif", "jpeg", "jpg", "m3u", "m4a", "md5", "ffp", "png", "webp",
];
static PLAYABLE_AUDIO_FILES: &[&str] = &["mp3", "flac", "ogg", "opus"];
static INDEXING_MINUTES: u64 = 10;
...
fn update_state_loop(state: State) {
    loop {
        ...
        thread::sleep(Duration::from_secs(60 * INDEXING_MINUTES));
    }
}
...
fn has_music_extension<P: AsRef<Path>>(path: P) -> bool {
    if let Some(ext) = path.as_ref().extension() {
        if let Some(ext) = ext.to_str() {
            ...
            if PLAYABLE_AUDIO_FILES.iter().any(|x| x == &extension) {
                return true;
            }
        }
    }
